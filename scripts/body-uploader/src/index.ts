...
import type { _SERVICE } from "../block_bodies.did.js";
...
async function processOneIteration(actor: _SERVICE, state: State): Promise<State> {
  ...
  const flush = async (): Promise<{ state: State; stop: boolean }> => {
    ...
    if ("err" in result) {
      logErr(
        `put_bodies heights=${firstH}..${lastH} (${pending.length} blocks, ` +
  ...
  for (let h = state.next; h <= end; h += 1) {
    ...
    if (txCount > MAX_BATCH_TXIDS) {
      ...
      if ("ok" in result) {
        const ok = result.ok;
        log(
          `put_body (oversize) height=${h} hash=${hashHex} tx_count=${txCount} ` +
