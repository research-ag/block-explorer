...
Poll-only variant of watch-headers.py. Same sync logic, but no ZMQ
subscriber — just `getblockcount` every POLL_INTERVAL seconds. Use
this when the Bitcoin node doesn't have ZMQ enabled (e.g. the
DAppNode Bitcoin package without zmqpubhashblock configured) and
you want quiet logs without the noisy reconnect spam.

Flow on every tick:
  1. Read bitcoind tip via getblockcount.
  2. If the tip moved since last tick: walk back LOOKBACK hashes,
     ask the canister `have_hashes` to find the common ancestor,
     fetch the missing 80-byte headers, push them in one
     `push_headers` call.
...
Requirements:
    pip install python-bitcoinrpc ic-py

bitcoin.conf (no ZMQ needed):
    server=1
    rpcuser=<user>
...
log = logging.getLogger("poll_headers")
...
def poll_loop(pusher: HeaderPusher) -> None:
    """Tick every POLL_INTERVAL seconds. _shutdown.wait() returns
    early on SIGINT/SIGTERM so the loop exits promptly."""
    log.info("Polling bitcoind every %.0fs", POLL_INTERVAL)
    while not _shutdown.is_set():
        pusher.sync()
        # wait() returns True if signal arrives, False on timeout
...
def main() -> int:
    log.info("poll_headers starting")
    ...
    try:
        ...
        log.exception("poll loop crashed: %s", e)
        return 1
